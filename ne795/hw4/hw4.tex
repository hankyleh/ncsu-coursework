...
\begin{equation}\label{eq:fourier-alpha}
  ...
  (\alpha_m)\delta\psi_{m, i+1/2}^{(s+1/2)} + (1 - \alpha_m )\delta\psi_{m, i-1/2}^{(s+1/2)} &= f_m^{(s+1/2)} \left( \alpha_m   e^{i\lambda\sigma_t x_{i-1/2}}     + \left(1-  \alpha_m\right)e^{i\lambda\sigma_t x_{i+1/2}}\right)\\
      ...
      &= f_m^{(s+1/2)} e^{i\lambda\sigma_t x_i}\left(\cos\beta - 2\alpha_m\sin\beta\right)
\end{split}
\end{equation}
...
\begin{equation}\label{eq:add-psi}
    \delta\psi_{m, i+1}^{(s+1/2)} + \delta\psi_{m, i-1}^{(s+1/2)} = f^{(s+1/2)}_m e^{i\lambda \sigma_t x_i}\left(2\cos(2\beta)  \right) \left(\cos\beta - 2\alpha_m\sin\beta\right)
\end{equation}
...
\begin{gather}
  2f_m^{(s+1/2)}\mu_m i\sin\beta + \sigma_t\Delta x \left(\cos\beta - 2\alpha_m i\sin\beta \right)f_m^{(s+1/2)} = \frac{\sigma_s \Delta x}{2}h^{(s)}\\
  f_m^{(s+1/2)} = h^{(s)} \frac{\sigma_s \Delta x}{2\left[(2i\sin\beta)(\mu_m - \sigma_t \Delta x \alpha_m) + \sigma_t \Delta x \cos\beta\right]}.
\end{gather}
