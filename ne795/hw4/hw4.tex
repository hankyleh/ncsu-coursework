...
\begin{gather}
  ...
  \label{eq:alpha-differencing}\delta\psi_{m, i}^{(s+1/2)} = \alpha_{m, i}\left( \delta \psi_{m,i-1/2}^{(s+1/2)} \right) + (1-\alpha_{m, i})\left( \delta\psi_{m,i+1/2}^{(s+1/2)} \right)\\
  (2a + \sigma_{a}\Delta x)\delta\phi_i^{(s+1)} -a (\delta\phi_{i-1}^{(s+1)} + \delta\phi_{i+1}^{(s+1)}) = \frac{1}{\sigma_t \Delta x} \left[\sum_{m=1}^{M}\left(\frac{1}{3} - \mu_m^2 \right) \left( 2\delta\psi_{m, i}^{(s+1/2)} - \delta\psi_{m, i+1/2}^{(s+1/2)} - \delta\psi_{m, i-1/2}^{(s+1/2)} \right) \right].
\end{gather}

The SM equation can also be rewritten using the SC differencing equation to eliminate $\psi_i$ dependence, reducing the system to two equations.

\begin{multline}
  (2a + \sigma_{a}\Delta x)\delta\phi_i^{(s+1)} -a (\delta\phi_{i-1}^{(s+1)} + \delta\phi_{i+1}^{(s+1)}) = \\
  \frac{1}{\sigma_t \Delta x} \left\{\sum_{m=1}^{M}\left(\frac{1}{3} - \mu_m^2 \right) \left( 2 \left[ \alpha_{m}\left( \delta \psi_{m,i-1/2}^{(s+1/2)} \right) + (1-\alpha_{m})\left( \delta\psi_{m,i+1/2}^{(s+1/2)} \right)\right] \right.\right.\\
  \left.\left.- \delta\psi_{m, i+1/2}^{(s+1/2)} - \delta\psi_{m, i-1/2}^{(s+1/2)} \right) \right\}
\end{multline}
...
\begin{equation}\label{eq:fourier-alpha}
  \begin{split}
  (\alpha)\delta\psi_{m, i+1/2}^{(s+1/2)} + (1 - \alpha )\delta\psi_{m, i-1/2}^{(s+1/2)} &= f_m^{(s+1/2)} \left( \alpha   e^{i\lambda\sigma_t x_{i-1/2}}     + \left(1-  \alpha\right)e^{i\lambda\sigma_t x_{i+1/2}}\right)\\
      &= f_m^{(s+1/2)} \left( \alpha \left(e^{i\lambda\sigma_t (x_i - \frac{1}{2}\Delta x)} - e^{i\lambda\sigma_t (x_i + \frac{1}{2}\Delta x)} \right) + e^{i\lambda\sigma_t (x_i + \frac{1}{2}\Delta x)}\right)\\
      &= f_m^{(s+1/2)} e^{i\lambda\sigma_t x_i}\left( \alpha \left(e^{-\frac{1}{2}i\lambda\sigma_t  \Delta x} - e^{\frac{1}{2}i\lambda\sigma_t  \Delta x} \right) + e^{\frac{1}{2}i\lambda\sigma_t \Delta x}\right)\\
      &= f_m^{(s+1/2)} e^{i\lambda\sigma_t x_i}\left( -2i\alpha\sin\left( \frac{1}{2} \lambda\sigma_t \Delta x\right) + e^{\frac{1}{2}i\lambda\sigma_t \Delta x}\right)
\end{split}
\end{equation}
...
Next, \autoref{eq:discrete-transport} is evaluated using using the Fourier Ansatz forms, and plugging in \autoref{eq:alpha-differencing} for $\delta\psi_{m, i}$ and using~\ref{eq:subtract-psi} and~\ref{eq:fourier-alpha} for the cell-edge terms:
